...
Tensor gpt2_forward(const GPT2Weights& m, const std::vector<int>& tokens) {
    ...
    if (T > c.n_ctx) throw std::invalid_argument("gpt2_forward: sequence longer than n_ctx");
    if (m.blocks.size() != c.n_layer)
        throw std::invalid_argument("gpt2_forward: wrong number of blocks for config");

    // Embedding: x[t] = wte[token_t] + wpe[t]. The token row says *what*
    // the token is, the position row says *where* it is; attention has
    // no notion of order by itself, so position must be added in.
    Tensor x({T, c.n_embd});
    for (size_t t = 0; t < T; ++t) {
        ...
        const float* pe = m.wpe.data() + t * c.n_embd;
